...
class PointsGridFormDataProvider extends ade_1.JSONSchemaFormDataProvider {
    constructor(config) {
        ...
        this.dimensions = lodash_1.default.get(this.data, "dimensions") || this._defaultDimensions;
        this.shifts = lodash_1.default.get(this.data, "shifts") || this._defaultShifts;
        // init class fields from data (as constructed from context in parent)
        this.gridMetricType = lodash_1.default.get(this.data, "gridMetricType") || "KPPRA";
        this.gridMetricValue =
            lodash_1.default.get(this.data, "gridMetricValue") || this._getDefaultGridMetricValue("KPPRA");
        this.preferGridMetric = lodash_1.default.get(this.data, "preferGridMetric", false);
    ...
    _getDefaultGridMetricValue(metric) {
        switch (metric) {
            case "KPPRA":
                return Math.floor(settings_1.globalSettings.defaultKPPRA / this._divisor);
            case "spacing":
                return 0.3;
            default:
    ...
    static _canTransform(data) {
        return ((data.preferGridMetric && data.gridMetricType && data.gridMetricValue) ||
            (!data.preferGridMetric && data.dimensions.every((d) => typeof d === "number")));
    }
    calculateDimensions({ gridMetricType, gridMetricValue, units = constants_1.units.angstrom }) {
    ...
    calculateGridMetric({ gridMetricType, dimensions, units = constants_1.units.angstrom }) {
        switch (gridMetricType) {
            case "KPPRA":
                return this._getKPPRAFromDimensions(dimensions);
            case "spacing":
                return lodash_1.default.round(this.reciprocalLattice.getSpacingFromDimensions(dimensions, units), 3);
            default:
                return 1;
        }
    }
...
(0, MaterialContextMixin_1.materialContextMixin)(PointsGridFormDataProvider.prototype);
